...
extern stmdev_ctx_t inertialHandle;
...
void Init_Accelerometer(void)
{
  ...
  BSP_SPI_Init();
	...
	lis2dw12_device_id_get(&inertialHandle, &WHO_AM_I);
	PRINTF("Who Am I %X", WHO_AM_I);
  
  /* Restore default configuration */
  lis2dw12_reset_set(&inertialHandle, PROPERTY_ENABLE);
  do {
    lis2dw12_reset_get(&inertialHandle, &rst);
  } while (rst);
  ...
  lis2dw12_block_data_update_set(&inertialHandle, PROPERTY_ENABLE);
  
  /* Set full scale */  
  lis2dw12_full_scale_set(&inertialHandle, LIS2DW12_2g);
  //lis2dw12_full_scale_set(&inertialHandle, LSM6DSOX_2000dps);
  
  /* Set Output Data Rate for Acc and Gyro */
  lis2dw12_data_rate_set(&inertialHandle, LIS2DW12_XL_ODR_25Hz);
  ...
  lsm6dsox_pin_int1_route_set(&inertialHandle, &int_1_reg);*/
	...
	//Accelerometer-filterpath / bandwidth
	lis2dw12_filter_path_get(&inertialHandle, LIS2DW12_LPF_ON_OUT);
	lis2dw12_filter_bandwidth_set(&inertialHandle, LIS2DW12_ODR_DIV_4);
	
	//configure power mode
	lis2dw12_power_mode_set(&inertialHandle, LIS2DW12_HIGH_PERFORMANCE);
...
#define SENSOR_PRESSURE_TEMPERATURE_EMULATION
#ifndef SENSOR_PRESSURE_TEMPERATURE_EMULATION
